...
AWS_PROFILE="uat-admin"

# Fetch DB_URL from AWS Secrets Manager
print_debug "Fetching DB_URL from AWS Secrets Manager..."
if ! DB_URL=$(aws secretsmanager get-secret-value --profile "${AWS_PROFILE}" --secret-id "hydrific-test-hydrific_db_url" --query SecretString --output text); then
...
BASTION_HOST="ec2-3-81-67-168.compute-1.amazonaws.com"
